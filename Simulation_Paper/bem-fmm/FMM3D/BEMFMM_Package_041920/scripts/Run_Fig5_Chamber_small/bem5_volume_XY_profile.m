...
delta   = 77.5e-3;                       %   half plane window, m    
...
save('temp_XY_Etot_big.mat', 'temp', 'x', 'y', 'EofXY', 'PofXY', 'countXY', 'xmin', 'xmax', 'ymin', 'ymax');
...
grid on; set(gcf,'Color','White');
set(gcf,"Position",[81 431 1014 1017]);
save('temp_XY_Epri_big.mat', 'temp', 'x', 'y', 'EofXY', 'PofXY');
...
grid on; set(gcf,'Color','White');
set(gcf,"Position",[81 431 1014 1017]);
save('temp_XY_Esec_big.mat', 'temp', 'x', 'y', 'EofXY', 'PofXY');
...
A = load('temp_XY_Etot_big.mat'); AA = reshape(A.temp, Ms, Ms); 
B = load('temp_XY_Epri_big.mat'); BB = reshape(B.temp, Ms, Ms); 
C = load('temp_XY_Esec_big.mat'); CC = reshape(C.temp, Ms, Ms); 
