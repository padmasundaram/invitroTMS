...
xlabel('Distance y, m');
ylabel('Distance z, m');
title(['Etot-field (V/m), ', label, '-component in the sagittal plane.']);
...
axis([yminzoom ymaxzoom zmin zmax]); clim([Emin,Emax]);
grid on;
L1 = line([-2e-3 2e-3],[1e-4 1e-4]); set(L1, 'LineStyle', '--', 'Color', 'k', 'LineWidth', 2);
set(gcf,"Position",figpos);
...
[yy,zz] = meshgrid(y,z); 
Sfig = figure('Color','w'); 
imagesc(y,z,reshape(Etotal(:, component),Ms,Ms)); clim([Emin,Emax]); setcolormap;
hold on; 
A = reshape(Etotal(:, 2),Ms,Ms);
B = reshape(Etotal(:, 3),Ms,Ms);
N = 25;
L = quiver(yy(1:N:end,1:N:end),zz(1:N:end,1:N:end),A(1:N:end,1:N:end),B(1:N:end,1:N:end));
