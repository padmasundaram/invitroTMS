...
bem4_define_planes_big;
bem5_figure_scales_big;
...
xlabel('Distance x, m');
ylabel('Distance y, m');
title(['Etot-field (V/m), ', label, '-component in the XY plane.']);
...
setcolormap; colorbar off; clim([Emin,Emax]);
...
% E-field plot: general settings 
axis 'equal';  axis 'tight';     
setcolormap; colorbar off; clim([Eprimin,Eprimax]);
axis([xmin xmax ymin ymax]);
grid on; set(gcf,'Color','White');
set(gcf,"Position", figpos);
EpriXY = temp;
...
Sfig = figure('Color','w'); 
imagesc(x,y,reshape(Etotal(:, component),Ms,Ms)); clim([Emin,Emax]); setcolormap;
hold on; 
%L = streamslice(xx,yy,reshape(Jx,Ms,Ms),reshape(Jy,Ms,Ms),6);
%L = quiver(xx,yy,reshape(Etotal(:, 1),Ms,Ms),reshape(Etotal(:, 2),Ms,Ms));
A = reshape(Etotal(:, 1),Ms,Ms);
B = reshape(Etotal(:, 2),Ms,Ms);
N = 50;
L = quiver(xx(1:N:end,1:N:end),yy(1:N:end,1:N:end),A(1:N:end,1:N:end),B(1:N:end,1:N:end));
set(L, 'Color', 'k', 'AutoScale', 'on', 'AutoScaleFactor', 1, 'LineWidth', 1);
...
box on;
axis([xmin xmax ymin ymax]);
set(gca, 'position', [0.1300 0.1100 0.7750 0.8150],'InnerPosition', [0.1300 0.1100 0.7750 0.8150], 'PlotBoxAspectRatio',[1,1,62.5]);
set(gcf,"Position",figpos);
xlabel('Distance x, m');
ylabel('Distance y, m');
set(gca, 'YDir', 'normal');
