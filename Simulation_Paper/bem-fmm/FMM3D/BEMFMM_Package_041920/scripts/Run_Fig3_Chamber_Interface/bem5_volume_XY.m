...
color   = prism(length(tissue)); color(:) = 0;
for m = countXY
...
setcolormap; colorbar; clim([Emin,Emax]);
axis([xmin xmax ymin ymax]);
grid on; set(gcf,'Color','White');
set(gcf,"Position",figpos);  %set(gcf,"Position",[81 431 1014 1017]);


Efig = figure; %subplot(132);
% E-field plot: contour plot
if component == 4
    temp      = abs(sqrt(dot(Etotal, Etotal, 2)));
else
...
% E-field plot: general settings 
axis 'equal';  axis 'tight';     
setcolormap; colorbar; clim([Eprimin,Eprimax]);
axis([xmin xmax ymin ymax]);
grid on; set(gcf,'Color','White');
set(gcf,"Position",figpos); %set(gcf,"Position",[81 431 1014 1017]);


Efig = figure; %subplot(133);
% E-field plot: contour plot
if component == 4
    temp      = abs(sqrt(dot(Etotal, Etotal, 2)));
else
...
grid on; set(gcf,'Color','White');
set(gcf,"Position",figpos); %set(gcf,"Position",[81 431 1014 1017]);
L1 = line([0 0], [-3e-3 3e-3]); set(L1, 'LineStyle', '--', 'Color', 'k', 'LineWidth', 2);
...
% hold on;
% L1 = line([-0.00207508 -0.00207508], [Emin Emax]); set(L1, 'LineStyle', '--', 'Color', 'k');
% L2 = line([+0.00207508 +0.00207508], [Emin Emax]); set(L2, 'LineStyle', '--', 'Color', 'k');
% L1 = line([-0.002 -0.002], [Emin Emax]); set(L1, 'LineStyle', '--', 'Color', 'k');
% L2 = line([+0.002 +0.002], [Emin Emax]); set(L2, 'LineStyle', '--', 'Color', 'k');
% xlabel('y (m)');
% ylim([Emin,Emax]);
